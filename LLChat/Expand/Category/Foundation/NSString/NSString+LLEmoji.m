...
#import "NSString+LLEmoji.h"

@implementation NSString (LLEmoji)
...
- (instancetype)ll_removedEmojiString {
    ...
    [self enumerateSubstringsInRange:NSMakeRange(0, [self length])
                             options:NSStringEnumerationByComposedCharacterSequences
                          usingBlock: ^(NSString* substring, NSRange substringRange, NSRange enclosingRange, BOOL* stop) {
                              [buffer appendString:([substring ll_stringContainsEmoji])? @"": substring];
                          }];
...
static NSDictionary * ll_s_unicodeToCheatCodes = nil;
static NSDictionary * ll_s_cheatCodesToUnicode = nil;

+ (void)ll_initializeEmojiCheatCodes
{
    ...
    @synchronized(self) {
        ll_s_unicodeToCheatCodes = forwardMap;
        ll_s_cheatCodesToUnicode = [reversedMap copy];
    }
}

- (NSString *)ll_stringByReplacingEmojiCheatCodesWithUnicode
{
    if (!ll_s_cheatCodesToUnicode) {
        [NSString ll_initializeEmojiCheatCodes];
    }
    
    if ([self rangeOfString:@":"].location != NSNotFound) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [ll_s_cheatCodesToUnicode enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
...
- (NSString *)ll_stringByReplacingEmojiUnicodeWithCheatCodes
{
    ...
    if (self.length) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [ll_s_unicodeToCheatCodes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
